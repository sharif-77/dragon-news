...
const Details = () => {
  ...
  const { title, image_url, details } = detailsData;

  useEffect(() => {
    const foundedData = data.find((data) => data._id == id);
    setDetailsData(foundedData);
  }, [data, id]);
  return (
    <div>
      <Navigation/>
      <div className="mt-10">
        <div className="relative flex  flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-md p-5">
          ...
          <div className="py-5">
            <p className="mt-3  font-sans text-xl font-normal leading-relaxed text-gray-700 ">
              {details}
            </p>
          </div>
